...
class Settings(BaseSettings):
    ...
    # Fixed server-side base system prompt (prompt-injection mitigation). A
    # cláusula de escopo do guardrail é acrescentada dinamicamente por
    # build_system_prompt (app/services/guardrail.py) quando
    # GUARDRAIL_SCOPE_ENABLED=true.
    ...
    # Guardrail de escopo temático, camada 1 (pré-filtro determinístico ANTES
    # do LLM): prompts sobre temas sensíveis conhecidos (política, religião
    # por padrão) recebem resposta controlada sem gastar tokens; a tentativa
    # fica auditável (status=blocked) e gera aviso (log WARNING +
    ...
    # Guardrail de escopo temático, camada 2 (escopo POSITIVO semântico): o
    # system prompt declara o escopo abaixo e o LLM sinaliza qualquer tema
    # divergente com a sentinela FORA_DO_ESCOPO — o serviço converte em
    # status=blocked com o mesmo aviso. Cobre temas que o pré-filtro por regex
    # não conhece (previsão do tempo, esportes, receitas...).
    guardrail_scope_enabled: bool = True
    guardrail_scope: str = (
        "indicadores e dados econômico-financeiros — câmbio e cotações (ex.: "
        "dólar PTAX, euro), juros (Selic, CDI), inflação (IPCA, IGP-M), "
        "índices e ativos de mercado (Ibovespa, ações, criptomoedas) e "
